    ...
    >
      ...
      <el-table :data="dataForm.transfees" border style="width: 100%;">
        <el-table-column header-align="center" align="center" width="450" label="可配送区域">
          <template slot-scope="scope">
            ...
            <span
              v-if="(!scope.row.addrNameList || !scope.row.addrNameList.length) && scope.$index > 0"
            >请选择可配送区域</span>
      ...
      >
        <el-table-column header-align="center" align="center" width="350" label="指定区域">
          <template slot-scope="scope">
            <span v-if="!scope.row.freeAddrNameList || !scope.row.freeAddrNameList.length">请选择指定包邮城市</span>
        ...
        <el-table-column header-align="center" align="left">
          <template slot-scope="scope">
            ...
            >
              ...
              >
                <template slot="prepend">满</template>
                <template slot="append">件/重量/体积包邮</template>
              </el-input>
            </el-form-item>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    checkNumber (row, type) {
      if (type === 1) {
        ...
        row.firstFee = this.getNumber(row.firstFee)
      } else if (type === 3) {
        row.continuousPiece = this.getNumber(row.continuousPiece)
      } else if (type === 4) {
        row.continuousFee = this.getNumber(row.continuousFee)
      } else if (type === 5) {
        row.amount = this.getNumber(row.amount)
      } else if (type === 6) {
        row.firstFee = this.getNumber(row.firstFee)
      }
    },
