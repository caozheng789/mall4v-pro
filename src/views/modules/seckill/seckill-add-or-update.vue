<template>
  <div>
    <el-dialog
      :title="!dataForm.seckillId ? '新增' : '查看'"
    ...
    >
      ...
      >
        ...
        <el-form-item label="活动标签" prop="seckillTag">
          <el-input
        ...
        <el-form-item label="每人限购" prop="maxNum">
          <el-checkbox
            v-model="hasMaxNum"
        ...
        <el-form-item label="订单取消" prop="maxCancelTime">
          买家，
          <el-input-number
        ...
        <el-form-item>
          ...
          >
            ...
            <div class="card-prod-bottom">
              ...
              <el-button type="text" class="card-prod-name-button" @click="deleteProd">删除</el-button>
            </div>
          </el-card>
        </el-form-item>
        <el-form-item v-if="skuList.length">
          <el-table :data="skuList" border style="width: 100%; margin-top: 20px">
            <el-table-column prop="prodName" label="商品名称" width="250">
              <template slot-scope="scope">{{scope.row.prodName}}</template>
            </el-table-column>
            <el-table-column prop="price" label="销售价" width="201">
              <template slot-scope="scope">
                ...
                ></el-input-number>
                <div>价格（元）：{{scope.row.price}}</div>
              </template>
            </el-table-column>
            <el-table-column prop="stocks" label="库存" width="201">
              <template slot-scope="scope">
                <el-input-number
                  type="number"
  ...
  methods: {
    ...
    checkSku () {
      ...
      this.skuList.forEach(element => {
        if (!element.seckillPrice) {
          this.isError = true
          this.value = '商品价格不能为空'
          return true
        }
        if (!element.seckillStocks && element.seckillStocks !== 0) {
          this.isError = true
          this.value = '商品库存不能为空'
          return true
        }
    ...
    dataFormSubmit () {
      ...
      console.log(this.dataForm.dateRange[0])
      console.log(this.dataForm.dateRange[1])
      if (this.dataForm.dateRange[1] <= this.dataForm.dateRange[0]) {
        this.$message({
