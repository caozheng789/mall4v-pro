<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="优惠券副标题" prop="subTitle">
        <el-input
      ...
      <el-form-item label="投放状态" size="mini" prop="putonStatus">
      ...
      <el-form-item label="优惠券类型" size="mini" prop="couponType">
      ...
      <el-form-item label="获取方式" size="mini" prop="getWay">
        <el-radio-group v-model="dataForm.getWay">
          <el-radio :label="0">直接领取</el-radio>
          <el-radio :label="1">兑换或系统发放</el-radio>
          <!-- <el-radio :label="3">兑换券</el-radio> -->
        </el-radio-group>
      </el-form-item>
      <el-form-item label="使用条件" prop="cashCondition">
        消费金额满
        <el-input
      ...
      <el-form-item label="减免金额" prop="reduceAmount" v-if="dataForm.couponType === 1">
        <el-input
      ...
      <el-form-item label="折扣额度" prop="couponDiscount" v-if="dataForm.couponType === 2">
        <el-input
          v-model="dataForm.couponDiscount"
          placeholder="请输入折扣额度 例：9.6为9.6折"
          type="number"
          :disabled="dataForm.couponId !== 0"
        >
      ...
      <el-form-item label="生效时间" np size="mini" prop="validTimeType">
      ...
      <el-form-item label="开始时间" prop="startTime" v-if="dataForm.validTimeType === 1">
        <el-date-picker
          :disabled="dataForm.couponId !== 0"
      ...
      <el-form-item label="结束时间" prop="endTime" v-if="dataForm.validTimeType === 1">
        <el-date-picker
      ...
      <el-form-item label="领券以后" prop="afterReceiveDays" v-if="dataForm.validTimeType === 2">
        <el-input
      ...
      <el-form-item label="有效天数" prop="validDays" v-if="dataForm.validTimeType === 2">
      ...
      <el-form-item label="库存" prop="stocks">
        <el-input v-model="dataForm.stocks" type="number">
          <template slot="append">张</template>
        </el-input>
      </el-form-item>
      <el-form-item label="适用商品" size="mini" prop="suitableProdType">
      ...
      <el-form-item style="width:100%" v-if="dataForm.suitableProdType !== 0">
        <el-row>
          <el-col :span="4" v-for="(couponProd, index) in dataForm.couponProds" :key="index">
...
export default {
  data () {
    ...
    var discountValidate = (rule, value, callback) => {
      if (!/^[1-9]\d*$|^[1-9]\d*\.\d\d?$|^0\.\d\d?$/.test(value)) {
        callback(new Error('请输入10以内的整数或者保留两位小数的正数'))
    ...
    return {
      ...
      dataRule: {
        ...
        afterReceiveDays: [
          { required: true, message: '领取后天数不能为空', trigger: 'blur' },
        ...
        couponDiscount: [
          { required: true, message: '折扣额度不能为空 例：9.6为9.6折', trigger: 'blur' },
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.dataForm.couponType === 1 && (parseFloat(this.dataForm.cashCondition) <= parseFloat(this.dataForm.reduceAmount))) {
            this.$message.error('减免金额不能大于或等于使用金额')
            return false
          }
          this.$http({
