<template>
  <el-dialog :title="!isEdit ? '查看' : '编辑'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form
      :model="dataForm"
      :rules="dataRule"
      ref="dataForm"
    ...
    >
      <div v-if="!isEdit">
        ...
        <el-form-item label="评论图片" prop="userName">
          <div v-if="dataForm.picsArray==null || dataForm.picsArray.length<1">无</div>
          <img
            max-width="100%"
            v-else
            v-for="item in dataForm.picsArray"
            :key="item"
