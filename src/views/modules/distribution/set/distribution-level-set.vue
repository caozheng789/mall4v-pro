<template>
  <div class="distribution-level-set gray-box bottom-redius">
    ...
    <el-form label-width="100px" class="set-form" size="mini" :model="dataForm">
      <el-form-item label="奖励设置">
        <el-checkbox v-model="dataForm.awardSwitch" :false-label="0" :true-label="1">默认奖励</el-checkbox>
        <el-checkbox v-model="dataForm.parentAwardSwitch" :false-label="0" :true-label="1">邀请人奖励</el-checkbox>
        <el-checkbox v-model="dataForm.awardProportion" :false-label="0" :true-label="1">按固定数值进行奖励</el-checkbox>
      </el-form-item>
      ...
      <el-form-item label="等级条件">
        ...
        <br />
        <span class="tips">至少需勾选一项，满足所有条件即可以升级</span>
        <el-table :data="dataForm.distributionLevels" class="elTable">
          <el-table-column prop="name" fixed label="等级名称" align="center">
          ...
          <el-table-column
            prop="conditionData.boundCustomers"
            label="绑定客户数(人)"
            align="center"
            min-width="160px"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.boundCustomers+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.boundCustomers'"
                  >
          ...
          <el-table-column
            prop="conditionData.invitedVeeker"
            label="邀请分销员数(人)"
            align="center"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.invitedVeeker+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.invitedVeeker'"
                  >
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.payNumber+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.payNumber'"
                  >
          ...
          <el-table-column
            prop="conditionData.addupAmount"
            label="实收金额(元)"
            align="center"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.addupAmount+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.addupAmount'"
                  >
          ...
          <el-table-column
            prop="conditionData.successOrderNumber"
            label="成交单数"
            align="center"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.successOrderNumber+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.successOrderNumber'"
                  >
          ...
          <el-table-column
            prop="conditionData.successTradingVolume"
            label="成交金额(元)"
            align="center"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.successTradingVolume+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.successTradingVolume'"
                  >
          ...
          <el-table-column
            prop="conditionData.sumOfConsumption"
            label="消费金额(元)"
            align="center"
          ...
          >
            <template slot-scope="scope">
              <div class="table-template">
                <div
                  v-if="scope.$index>0"
                >{{dataForm.distributionLevels[scope.$index-1].conditionData.sumOfConsumption+0.01}}</div>
                <div v-else>0</div>
                <div>~</div>
                <div class="table-input-box">
                  <el-form-item
                    :prop="'distributionLevels.' + scope.$index + '.conditionData.sumOfConsumption'"
                  >
          ...
          >
            <template slot-scope="scope">
              ...
              <div class="card-prod-container">
                <div v-for="(item, index) in scope.row.conditionData.commodity" :key="index">
                  <el-card :body-style="{ padding: '0px' }" style="height: 160px;width: 120px">
                    <img :src="item.pic" style="height:104px;width:100%" />
                    <div class="card-prod-bottom">
                      <span class="card-prod-name">{{item.prodName}}</span>
                      <el-button
                        type="text"
                        class="card-prod-name-button"
                        @click="deleteProd(scope.row.conditionData.commodity,index)"
                      >删除</el-button>
                    </div>
                  </el-card>
                </div>
              ...
              <el-button
                @click.native.prevent="prodsSelectHandle(scope.$index)"
                type="text"
                size="small"
              >选择商品</el-button>
            </template>
              ...
              <div class="table-template">
                <div class="table-input-box award-box">
                  <el-form-item :prop="'distributionLevels.' + scope.$index + '.awardNumber'">
                    <el-input v-model.number="scope.row.awardNumber" style="width:100px">
              ...
              <div class="table-template">
                <div class="table-input-box award-box">
                  <el-form-item :prop="'distributionLevels.' + scope.$index + '.parentAwardNumber'">
                    <el-input v-model.number="scope.row.parentAwardNumber" style="width:100px">
                      <template slot="append">
                        <span v-if="dataForm.awardProportion === 0">%</span>
                        <span v-else>元</span>
                      </template>
