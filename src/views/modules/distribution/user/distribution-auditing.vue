<template>
  <div class="mod-distribution-auditing">
    <avue-crud
      ...
      @on-load="getDataList"
    >
      <template slot-scope="scope" slot="parentNickName">{{scope.row.parentNickName}}</template>

      <template slot-scope="scope" slot="sysUsername">
        <div v-if="scope.row.modifier === 0">系统</div>
        <div v-if="scope.row.sysUsername != null">{{scope.row.sysUsername}}</div>
        <div v-if="scope.row.sysUsername === null">无</div>
      </template>

      <template slot-scope="scope" slot="sumOfConsumption">
        <span v-if="scope.row.sumOfConsumption === null">0</span>
        <span v-else>{{scope.row.sumOfConsumption}}</span>
      </template>

      <template slot-scope="scope" slot="expenseNumber">
        <span v-if="scope.row.expenseNumber === null">0</span>
